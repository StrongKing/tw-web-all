...
const RichText = ({
  ...
  const encodeHis = () => {
    // 还原历史记录
    const list = JSON.parse(window.localStorage.getItem(RICHTEXT_KEY));
    list?.map((item, i) => {
      const d = item.split('|');
      const o = main[d[0]][d[1]][d[2]];
      const key = d[3];
      if (o.key === key) {
        o.click = d[4];
      }
    ...
    const _lib = [...[[_hisC, _hisE]], ...main];
    return _lib;
  };

  var [main, hisC, hisE] = initData(MATLIB(ossUrl));
  const LIB = initLib(main, hisC, hisE);
  ...
  useEffect(() => {
    ...
    editor.config.uploadImgServer = `${requestUrl}/web/file/uploadReturnUrl`;
    // 限制一次最多上传 1 张图片
    editor.config.uploadImgMaxLength = 1;
    editor.config.uploadVideoServer = `${requestUrl}/web/file/uploadReturnUrl`;
    editor.config.customAlert = function (s, t) {
      switch (t) {
        ...
        case 'warning':
          if (s.includes('视频')) {
            message.warning('错误类型，视频支持格式为mp4');
            return;
          }
          if (s.includes('图片')) {
            message.warning('错误类型，图片支持格式为：jpg、jpeg、png、gif');
            return;
          }
          message.warning(s);
          break;
    ...
    editor.config.customUploadImg = function (files, insert) {
      ...
      if (files[0]) {
        ...
        request(`${requestUrl}/web/file/uploadReturnUrl?appCode=${appCode}`, {
          method: 'POST',
          data: formData,
        })
          .then((data) => {
  ...
  return (
    <Spin tip="Loading..." spinning={loading}>
      <div
        className="g-rt"
        style={{
          minHeight: showQuickMenu ? minHeight : undefined,
        }}
      >
        {showQuickMenu && (
          <div className="m-bar">
