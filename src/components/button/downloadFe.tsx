...
export const downloadFeButtonHOC =
  (Comp: any) =>
  ({
    onSuccess,
    onError,
    ...
    filename = 'download',
    successCodes = [],
    dataFormatter = (result: any) => result?.data,
    ...others
  }: RequestButtonProps & {
    smsApiUrl?: string;
    importHeaders: {
    ...
    const handleBeforeClick = async (arg = { terminate: () => {} }) => {
      if (onBeforeClick) {
    ...
    const handleSuccess = (result: any) => {
      ...
      let dataSource = [];
      if (!result || !Array.isArray((dataSource = dataFormatter(result)))) {
        console.error('下载接口返回数据错误');
        onError && onError(new Error('下载接口错误，请联系管理员'));
        return;
      }
      let csvContent = importHeaders.map((el) => el.label).join(',') + '\n';
      dataSource.forEach((row: any) => {
        csvContent +=
          importHeaders.map((el) =>
            typeof el.formatter === 'function'
              ? el.formatter(row[el.key], row)
              : row[el.key],
          ) + '\n';
      });
      // 跳转到下载链接
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute(
          'download',
          `${filename}_${moment().format('YYYYMMDDHHmmss')}.csv`,
        );
...
export default downloadFeButtonHOC(RequestButton);
