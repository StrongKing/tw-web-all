...
export default ({
  ...
  function getModuleType(type: string) {
    return (extendModuleMap && extendModuleMap[type]) || moduleMap[type];
  }
        ...
        .then(
          ({ data, ...others }: { data: { dataSource: TreeDataSource } }) => {
            ...
            if (!data) {
              setDataCode(code);
              return;
            } else if (data?.dataSource.length === 0) {
              if (!id || id === 'null') {
                setDataCode(9000);
              }
